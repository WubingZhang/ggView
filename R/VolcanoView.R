...
#' @param x Colname of mat specifying x-axis in Volcanno figure.
#' @param y Colname of mat specifying y-axis in Volcanno figure.
#' @param Label Colname of mat specifying labeled terms in Volcanno figure.
#' @param top Interger, label top significant terms.
...
#' @param ylab Label of t-axis in figure.
...
VolcanoView <- function(mat, x = "logFC", y = "nLogP", Label=NA, top = 0,
                        topnames = NULL, filename = NULL,
                        x_cutoff = 0.1, y_cutoff = -log10(0.05), main = NULL,
  ...
  if(!(top==0 & is.null(topnames)))
    p = p + ggrepel::geom_label_repel(aes(x=gg[idx,x],y=gg[idx,y], fill=group,label = Label), data=gg[idx,],
                             fontface = 'bold', color = '#EBB38A',size = 2.5,
