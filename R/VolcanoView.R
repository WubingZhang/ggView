...
VolcanoView <- function(mat, x = "logFC", y = "adj.P.Val", Label=NA, top = 0,
  ...
  mycolour=c("no"="aliceblue",  "up"="OrangeRed","down"="RoyalBlue")
  #=========
  ...
  p = p + annotate("text",color="OrangeRed",x=x_cutoff, y=max(gg[,y]), hjust = 0, vjust = 1,
                   label=paste("Up: ",dim(gg[gg$group=="up",])[1],sep=""))
  p = p + annotate("text",color="RoyalBlue",x=(-x_cutoff), y=max(gg[,y]),hjust = 1, vjust = 1,
