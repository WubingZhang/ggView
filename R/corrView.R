...
corrView <- function(mat, geneset1, geneset2, xlab = geneset1[1],
  ...
  corr = paste('r = ', round(tmp$estimate,3),
               "\np = ", signif(tmp$p.value,3),
               "\nn = ", nrow(gg))
  p = ggplot(gg, aes(x, y))
  p = p + geom_point(size=0.5, color="blue")
  p = p + geom_smooth(method='glm',se=TRUE, size=0.5, color = "#c12603")
  ...
  if(tmp$estimate>0){
    p = p + annotate("text", x = min(gg$x), y = max(gg$y),
                     label = corr, hjust=0, vjust=1)

  }else{
    p = p + annotate("text", x = min(gg$x), y = min(gg$y),
                     label = corr, hjust=0, vjust=0)
  }
  p
}
