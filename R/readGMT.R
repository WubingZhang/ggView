...
readGMT <- function(gmtpath, limit = c(3, 30)){
  ...
  c2_list = strsplit(c2, "\t")
  c2_pathway = unlist(lapply(c2_list, function(x) x[1]))
  c2_list = lapply(c2_list, function(x) x[-(1:2)])
  names(c2_list) = c2_pathway
  c2_len = lapply(c2_list, length)
  gene2path = data.frame(Gene = unlist(c2_list), Pathway = rep(names(c2_list), c2_len))
  limit_pathways = names(c2_list)[c2_len>limit[1] & c2_len<limit[2]]
  gene2path = gene2path[gene2path$Pathway%in%limit_pathways, ]
  return(gene2path)
}
