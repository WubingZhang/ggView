...
Figure1H <- function(m2, SampleAnn, sig = "sgTRAF3",
  ...
  pval <- signif(wilcox.test(tmpDat[, sig] ~ tmpDat[, biResp])$p.value, 2)
  print(paste("Wilcoxon test P", sig, "score by binary Response:", pval))
  pval <- signif(t.test(tmpDat[, sig] ~ tmpDat[, biResp])$p.value, 2)
  ...
  mtext(paste0("Response ~ ", sig), side=3, at=2.5, line=1, font=titleF, cex=titleCex)
