...
#' @param type Geneset category for testing, KEGG(default) or GMT (gmtpath is required).
...
#' @param gmtpath The file path of GMT genesets.
#'
...
enrichHGT = function(geneList, universe=NULL, type="KEGG", organism='hsa', pvalueCutoff = 0.25,
                      pAdjustMethod = "BH", minGSSize = 2, maxGSSize = 500, gmtpath = NA){
  requireNamespace("data.table", quietly=TRUE) || stop("need data.table package")
  requireNamespace("clusterProfiler", quietly=TRUE) || stop("need data.table package")
  # download Kegg data
  ...
  if(type=="GMT"){
    gene2path = readGMT(gmtpath)
    names(gene2path)=c("EntrezID","PathwayID")
    gene2path$EntrezID = TransGeneID(gene2path$EntrezID)
    gene2path = gene2path[!is.na(gene2path$EntrezID), ]
    pathways = data.frame(PathwayID = unique(gene2path$PathwayID),
                          PathwayName = unique(gene2path$PathwayID))
  }
  ##==========
  gene = names(geneList)
